...
def receive_control_data(connection_socket):
    # Define the format of the struct
    format = 'fffif'  # 4 floats, 1 bool, 1 int

    # Calculate the size of one tuple
    tuple_size = struct.calcsize(format)
    ...
    for i in range(0, len(data), tuple_size):
        tuple_data = data[i:i+tuple_size]
        control_data = struct.unpack(format, tuple_data)
        control_data = ControlData(*control_data)

        # Put the control data in the queue
...
try:
    ...
    command_being_executed = False
    while True:
        
        ready_to_read, _, _ = select.select([connection_socket], [], [], 0)
        
        if ready_to_read:
            print("ready to read")
            receive_control_data(connection_socket)
        ...
        if control_data_queue:
            ...
            #next_time = current_time + control_data.time
            ...
            if control_data.time != 0.0:
                current_time = time.time()
                time.sleep(control_data.time)
                current_time1 = time.time()
                print("Control data applied", current_time1 , current_time)
