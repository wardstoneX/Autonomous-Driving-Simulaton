...
/* ARM architecture as used on Raspberry Pi is little endian */
#define LITTLE_ENDIAN_ORDER

/* Functions to do the byte reversal, as copied from wolfTPM's tpm2_types.h
 * I removed most of the macro tests, since they do not apply here - we only
 * need to support Raspberry Pi, but kept a check for whethere we are compiling
 * with GCC, since I don't know whether the library might ever be compiled
 * with other compilers.
 *
 * There were problems compiling with the __GCC_PREREQ macro that wolfTPM uses
 * for this check, and according to this comment:
 *   https://github.com/besser82/libxcrypt/issues/40#issuecomment-433767995
 * that's not the proper macro for this use anyway, so I just check that
 * with __GCC__.
 */
static inline uint16_t ByteReverseWord16(uint16_t value)
{
#if defined(__GNUC__)
...
{
#if defined(__GNUC__)
    return (uint64_t)__builtin_bswap64(value);
#else
    return (uint64_t)((uint64_t)ByteReverseWord32((uint32_t)value)) << 32 |
                    (uint64_t)ByteReverseWord32((uint32_t)(value  >> 32));
#endif
}
...
static inline void c16toa(uint16_t wc_u16, char* c)
{
    c[0] = (wc_u16 >> 8) & 0xff;
    c[1] =  wc_u16 & 0xff;
}
/* convert 32 bit integer to opaque */
static inline void c32toa(uint32_t wc_u32, char* c)
{
    c[0] = (wc_u32 >> 24) & 0xff;
    c[1] = (wc_u32 >> 16) & 0xff;
    c[2] = (wc_u32 >>  8) & 0xff;
...
{
    if (packet && (packet->pos + (int)sizeof(uint8_t) <= packet->size)) {
        packet->buf[packet->pos] = data;
        packet->pos += sizeof(uint8_t);
    }
}
...
{
    if (packet && (packet->pos + (int)sizeof(uint16_t) <= packet->size)) {
        data = cpu_to_be16(data);
        memcpy(&packet->buf[packet->pos], &data, sizeof(uint16_t));
...
void TPM2_Packet_ParseU16(TPM2_Packet* packet, uint16_t* data)
{
    uint16_t value = 0;
    if (packet && (packet->pos + (int)sizeof(uint16_t) <= packet->size)) {
        memcpy(&value, &packet->buf[packet->pos], sizeof(uint16_t));
...
{
    if (packet && (packet->pos + (int)sizeof(uint32_t) <= packet->size)) {
        data = cpu_to_be32(data);
        memcpy(&packet->buf[packet->pos], &data, sizeof(uint32_t));
...
void TPM2_Packet_ParseU32(TPM2_Packet* packet, uint32_t* data)
{
    ...
    if (packet && (packet->pos + (int)sizeof(uint32_t) <= packet->size)) {
        if (data) {
            memcpy(&value, &packet->buf[packet->pos], sizeof(uint32_t));
...
{
    if (packet && (packet->pos + (int)sizeof(uint64_t) <= packet->size)) {
        data = cpu_to_be64(data);
        memcpy(&packet->buf[packet->pos], &data, sizeof(uint64_t));
...
void TPM2_Packet_ParseU64(TPM2_Packet* packet, uint64_t* data)
{
    ...
    if (packet && (packet->pos + (int)sizeof(uint64_t) <= packet->size)) {
        if (data) {
            memcpy(&value, &packet->buf[packet->pos], sizeof(uint64_t));
...
{
    if (packet && (packet->pos + (int)sizeof(int32_t) <= packet->size)) {
        data = cpu_to_be32(data);
        memcpy(&packet->buf[packet->pos], &data, sizeof(int32_t));
        packet->pos += sizeof(int32_t);
    }
}
...
void TPM2_Packet_AppendBytes(TPM2_Packet* packet, char* buf, int size)
{
    if (packet && (packet->pos + size <= packet->size)) {
        if (buf)
            memcpy(&packet->buf[packet->pos], buf, size);
        packet->pos += size;
    }
}
void TPM2_Packet_ParseBytes(TPM2_Packet* packet, char* buf, int size)
{
    if (packet) {
        if (buf) {
            ...
            memcpy(buf, &packet->buf[packet->pos], sizeToCopy);
        }
        packet->pos += size;
    }
}
...
int TPM2_Packet_PlaceU16(TPM2_Packet* packet, int markSz)
{
    ...
    if (packet) {
        uint16_t data;
        char* sizePtr = &packet->buf[markSz];
        markSz += sizeof(uint16_t); /* skip marker */
        if (markSz <= packet->pos) {
            rc = packet->pos - markSz;

            data = cpu_to_be16(rc);
            memcpy(sizePtr, &data, sizeof(uint16_t));
        }
    }
    return rc;
}
...
void TPM2_Packet_PlaceU32(TPM2_Packet* packet, int markSz)
{
    ...
    if (packet) {
        uint32_t data;
        char* sizePtr = &packet->buf[markSz];
        markSz += sizeof(uint32_t); /* skip marker */
        if (markSz <= packet->pos) {
            markSz = packet->pos - markSz;

            data = cpu_to_be32(markSz);
            memcpy(sizePtr, &data, sizeof(uint32_t));
        }
    }
}
...
{
    uint32_t cmdSz = packet->pos; /* get total packet size */
